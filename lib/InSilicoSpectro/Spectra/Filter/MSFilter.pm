    ...
    if ( ref( $this->currentSpectra() ) eq 'InSilicoSpectro::Spectra::MSRun' ) {
        ...
        unless ( defined $this->{fragPD} ) {
            if ( ref($sp) eq 'InSilicoSpectro::Spectra::MSMSSpectra' ) {
                ...
                $this->{fragPD}->{intensity} =
                  $this->currentSpectra()->get('fragPD')
                  ->getFieldIndex('intensity');
                $this->{fragPD}->{moz} =
                  $this->currentSpectra()->get('fragPD')->getFieldIndex('moz');
                ( $this->{fragPD}->{charge} =
                      $this->currentSpectra()->get('fragPD')
                      ->getFieldIndex('charge') )
                  or ( $this->{fragPD}->{chargemask} =
                    $this->currentSpectra()->get('fragPD')
                    ->getFieldIndex('chargemask') );

                $this->{parentPD}->{intensity} =
                  $this->currentSpectra()->get('parentPD')
                  ->getFieldIndex('intensity');
                $this->{parentPD}->{moz} =
                  $this->currentSpectra()->get('parentPD')
                  ->getFieldIndex('moz');
                ( $this->{parentPD}->{charge} =
                      $this->currentSpectra()->get('parentPD')
                      ->getFieldIndex('charge') )
                  or ( $this->{parentPD}->{chargemask} =
                    $this->currentSpectra()->get('parentPD')
                    ->getFieldIndex('chargemask') );
                ...
                #if it's a MSSpectra
                $this->{fragPD}->{intensity} =
                  $this->currentSpectra()->get('peakDescriptor')
                  ->getFieldIndex('intensity');
                $this->{fragPD}->{moz} =
                  $this->currentSpectra()->get('peakDescriptor')
                  ->getFieldIndex('moz');
                $this->{fragPD}->{chargemask} =
                  $this->currentSpectra()->get('peakDescriptor')
                  ->getFieldIndex('chargemask');
        ...
        if (    ( $this->{level} eq 'peaks' )
            and ( $this->{spectrumType} eq 'msms' )
            and ( ref($sp) ne 'InSilicoSpectro::Spectra::MSMSCmpd' ) )
        {

            my $sp_backup = $this->currentSpectra();
            if (
                ref( $this->currentSpectra() ) eq
...
sub normRankBern {
    ...
    foreach ( 0 .. $#{ $this->filterValue() } ) {
        $this->filterValue()->[$_] =
          $c1 - ( $c2 / $max_mz ) * $rank{ $this->filterValue()->[$_] };
...
sub normRank {
    ...
    foreach ( 0 .. $#{ $this->filterValue() } ) {
        $this->filterValue()->[$_] =
          $rank{ $this->filterValue()->[$_] } / $rank_nr;
    }
}
...
sub applyAction {
    ...
    if (   ( $this->{actionType} eq 'none' )
        or ( $this->{actionType} eq 'algorithm' ) )
    {
        return $this;
    }
    ...
    my @keep =
      map { eval "$_ $comparator $threshold" } @{ $this->filterValue() };
    carp($@) if $@;

    if ( $this->{actionType} eq 'label' ) {

        croak "you cannot label fragments\n"
          unless ref( $this->currentSpectra() ) eq
              "InSilicoSpectro::Spectra::MSMSSpectra";

        my $i = 0;
        foreach ( @{ $this->currentSpectra()->spectrum() } ) {
            my $value_tmp =
              sprintf( "$this->{labelValue}", $this->filterValue()->[$i] );
...
sub readXml {
    ...
    my $twig = XML::Twig->new(
        twig_handlers => {
            '/ExpMsMsSpectrumFilter/oneExpMsMsSpectrumFilter' =>
              sub { twig_addSpectrumFilter( $this, $_[0], $_[1] ) },
        },
...
sub twig_addSpectrumFilter {
    ...
    if ( $this->{actionType} eq 'label' ) {
        $this->{labelValue} =
          $el->first_child('action')->first_child('labelValue')->text;
        $this->{labelName} =
          $el->first_child('action')->first_child('labelName')->text;
    }

    unless ( ( $this->{actionType} eq 'algorithm' )
        or ( $this->{actionType} eq 'none' ) )
    {

...
sub smartPeaks {
      ...
      )
    {
        ...
        my $mean =
          $sum /
          ( ( scalar @actual_window )**( 1 / $this->param()->{weightDensity} )
          );
    ...
    if ( $this->{actionType} eq 'algorithm' ) {
        foreach ( 0 .. $#int ) {
            $this->currentSpectra()->spectra()->[$_]
              ->[ $this->{fragPD}->{intensity} ] = $int[$_];
        }
    }
...
sub balance {
    ...
    if (
        ref( $this->currentSpectra() ) ne 'InSilicoSpectro::Spectra::MSMSCmpd' )
    {
        ...
        my ( $min_moz, $max_moz );
        ( $this->param()->{minMoz} ) > ( min(@moz) )
          ? ( $min_moz = $this->param()->{minMoz} )
          : ( $min_moz = min(@moz) );
        ( $this->param()->{maxMoz} ) < ( max(@moz) )
          ? ( $max_moz = $this->param()->{maxMoz} )
          : ( $max_moz = max(@moz) );
        ...
        for (
            my $i = $min_moz ;
            &compare_float( $i + $part_size, '<=', $max_moz, $precision ) ;
            $i += $part_size
          )
        {
...
sub compare_float {
    ...
    my $result =
      eval "sprintf(\"%.${dp}g\", $A) $comparator sprintf(\"%.${dp}g\", $B)";
    carp($@) if $@;
...
sub balanceBern {
        ...
        ref( $this->currentSpectra() ) ne 'InSilicoSpectro::Spectra::MSMSCmpd' )
    {
        ...
        for ( my $i = $min_moz ;
            $i + $part_size <= $max_moz ; $i += $part_size )
        {
...
sub goodDiff {
        ...
        ref( $this->currentSpectra() ) ne 'InSilicoSpectro::Spectra::MSMSCmpd' )
    {
        ...
        foreach my $a_cmpnd ( @{ $this->currentSpectra()->spectrum() } ) {
            ...
            my @double =
              @{ $this->originalFilterName() }
              [ 1 .. $#{ $this->originalFilterName() } ];
        ...
        if ( !defined $this->{aaMasses} ) {
            ...
            foreach (@aa_list) {
                push @mono,
                  InSilicoSpectro::InSilico::MassCalculator::getMass( "aa_$_",
                    0 );
                push @average,
                  InSilicoSpectro::InSilico::MassCalculator::getMass( "aa_$_",
                    1 );
        ...
        for ( my $current_peak = 0 ; $current_peak < $#moz ; $current_peak++ ) {
            ...
            for ( my $j = 0 ; $j <= $#{ $this->{aaMasses}->{$mass_id} } ; $j++ )
            {
                ...
                for (
                    my $i = $current_peak + 1 ;
                    (
                        $moz[$i] - $moz[$current_peak] <=
                          $this->{aaMasses}->{$mass_id}->[$j] + $tolerance
                      )
                      and $i <= $#moz ;
                    $i++
                  )
        ...
        ref( $this->currentSpectra() ) ne 'InSilicoSpectro::Spectra::MSMSCmpd' )
    {
        ...
        for ( my $current_peak = 0 ; $current_peak < $#moz ; $current_peak++ ) {
            ...
            for (
                ...
                (
                    $moz[$i] - $moz[$current_peak] <=
                      $waterMasses{$mass_id} + $tolerance
                  )
                  and $i <= $#moz ;
                $i++
              )
            {
                if ( $moz[$i] - $moz[$current_peak] >=
                    $waterMasses{$mass_id} - $tolerance )
                {
...
sub selectPeakWindow {
    ...
    if ( $action_type eq 'algorithm' ) {
        ...
        for ( my $j = 0 ; $j < $window_nr ; $j++ ) {
            ...
            my $threshold =
              ( ( sort { $b <=> $a } @win_int )[ $peak_nr_window - 1 ] );
        ...
        @{ $this->currentSpectra()->spectra } =
          @{ $this->currentSpectra()->spectra }[@to_keep];
...
sub banishNeighbors {
    ...
    my $skip_spectra_below = $this->param()->{skipSpectraBelow}
      if ( $action_type eq 'algorithm' );
    ...
    foreach ( 0 .. $#keep ) {
        push @selected_peaks_moz,
          $this->currentSpectra()->spectra()->[$_]->[ $this->{fragPD}->{'moz'} ]
          if $keep[$_];
    }
    ...
    foreach my $j ( 0 .. $#selected_peaks_moz ) {
        my $left_border =
            $selected_peaks_moz[$j] - $win_size >= $min_moz
          ? $selected_peaks_moz[$j] - $win_size
          : $min_moz;
        my $right_border =
            $selected_peaks_moz[$j] + $win_size <= $max_moz
          ? $selected_peaks_moz[$j] + $win_size
          : $max_moz;

        #ze window
        my @current_win =
          map { ( $_ >= $left_border ) and ( $_ <= $right_border ) } @moz;
        ...
        foreach ( keys %current_win_values ) {
            if ( $action_type eq 'algorithm' ) {
                ...
                $int[$_] = $min_int / $ranks[$_]
                  if ( $current_win_values{$_} < $peak_limit );
            }
...
sub complements {
        ...
        ref( $this->currentSpectra() ) ne 'InSilicoSpectro::Spectra::MSMSCmpd' )
    {
        ...
        my $parent_moz =
          $this->currentSpectra()->getParentData( $this->{parentPD}->{moz} );

        if ( defined $this->{parentPD}->{chargemask} ) {
            my $mask =
              $this->currentSpectra()
              ->getParentData( $this->{parentPD}->{chargemask} );
            ...
            @parent_charge = split( ',',
                $this->currentSpectra()
                  ->getParentData( $this->{parentPD}->{charge} ) );
        }
        ...
        for ( my $current_peak = 0 ; $current_peak < $#moz ; $current_peak++ ) {
            ...
            for ( my $i = $current_peak + 1 ; $i <= $#moz ; $i++ ) {
                ...
                foreach (@parent_charge) {
                    ...
                    if (    ( $sum_moz >= ( $parent_moz * $_ ) - $tolerance )
                        and ( $sum_moz <= ( $parent_moz * $_ ) + $tolerance ) )
                    {

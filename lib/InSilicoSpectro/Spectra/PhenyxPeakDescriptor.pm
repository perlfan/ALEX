...
sub readTwigEl {
    ...
    confess unless $t;
    confess
      "InSilicoSpectro::Spectra::PhenyxPeakDescriptor::readTwigEl tag name should be (ple:)?ItemOrder"
      unless $t->gi =~ /(ple:)?ItemOrder/;
    $this->{fieldNames} = [];
    foreach ( $t->children("ple:item"), $t->children("item") ) {
        my $f = $_->att('type')
          or croak
          "InSilicoSpectro::Spectra::PhenyxPeakDescriptor::readTwigEl no 'type' attribute for element ple:item";
        $this->pushField($f);
    }
}
    ...
    foreach ( @{ $this->{fieldNames} } ) {
        ...
        if ( $transformCharge and ( $_ eq 'chargemask' ) ) {
            if ( $v eq '?' or not $v ) {
                ...
                $ret .=
                  InSilicoSpectro::Spectra::MSSpectra::chargemask2string($v)
                  . " ";
            }
        }
        else {

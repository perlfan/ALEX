...
@ISA =
  qw (InSilicoSpectro::Utils::FileCached InSilicoSpectro::Spectra::ExpSpectrum );
@EXPORT =
  qw(&string2chargemask &chargemask2string &charge2mgfStr &guessFormat &getFmtDescr $USE_FILECACHED);
@EXPORT_OK = qw();
...
sub new {
    ...
    my $spec =
      $USE_FILECACHED
      ? InSilicoSpectro::Utils::FileCached->new( persistent => $h{persistent} )
      : {};
    $spec->{spectra} = [];
    bless $spec, $class;
    if (   ref( $_[0] )
        && ( ref( $_[0] ) ne 'HASH' )
        && $_[0]->isa('InSilicoSpectro::Spectra::ExpSpectrum') )
    {
        %$spec = %{ $_[0] };
    }
    elsif (ref( $_[0] )
        && ( ref( $_[0] ) ne 'HASH' )
        && $_[0]->isa('InSilicoSpectro::Spectra::MSSpectra') )
    {
        %$spec = %{ $_[0] };
    }
    else {
...
sub open {
    ...
    my $fmt = $this->format;
    InSilicoSpectro::Utils::io::croakIt "unknown spectra format [$fmt]"
      unless defined $hFmt{$fmt};
...
sub guessFormat {
    ...
    if ( ( ref $this ) eq 'InSilicoSpectro::Spectra::MSSpectra' ) {
        ...
        $this->source($src)
          or croak
          "InSilicoSpectro::Spectra::MSSpectra:guessFormat not possible as not source was defined";
        $isobj = 1;
    }
    else {
        $src = $this;
    ...
    croak
      "InSilicoSpectro::Spectra::MSSpectra:guessFormat not possible as not source [$src] is not within the known formats";
}
...
sub writePLE {
    ...
    print
      "$shift<ple:PeakListExport spectrumType=\"pmf\" xmlns:ple=\"http://www.phenyx-ms.com/namespaces/PeakListExport.html\">
...
";
    ...
    if ( defined $this->get('sampleInfo') ) {
        ...
        Carp::confess "No sample info available when saving to ple "
          . $this->source;
    }
    ...
    print "$shift      <ple:description><![CDATA["
      . $this->title
      . "]]></ple:description>\n";
...
sub read {
    ...
    my $fmt = $this->format();
    Carp::confess
      "InSilicoSpectro::Spectra::MSSpectra: no reading handler is defined for format [$fmt]"
...
sub readTwigEl {
    ...
    $this->set( 'title',
        ( $elpl->get_xpath("ple:MSRun/ple:description") )[0]->text );

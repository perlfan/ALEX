...
@ISA = qw(Exporter);
@EXPORT =
  qw(normalizeIntensities annotatePept chooseColorLineNum chooseColorFrag htmlCSS plotLegendOnly );
@EXPORT_OK = ();
...
sub new {
    ...
    my (%h) = @_;
    my (
        $theo,      $prec,   $modifLvl, $cmp,
        $spec,      $intSel, $tol,      $minTol,
        $massIndex, $intensityIndex
      )
      = (
        $h{spectrum}, $h{prec},        $h{modifLvl},
        $h{cmp},      $h{expSpectrum}, $h{intSel},
        $h{tol},      $h{minTol},      $h{massIndex},
        $h{intensityIndex}
      );
    $prec = $prec || 3;
    my $format = '%.' . $prec . 'f';
    ...
    if (   ref($spec)
        && ( ref($spec) ne 'ARRAY' )
        && $spec->isa('InSilicoSpectro::Spectra::ExpSpectrum') )
    {
    ...
    if (   ref($theo)
        && ( ref($theo) ne 'HASH' )
        && $theo->isa('InSilicoSpectro::InSilico::MSMSTheoSpectrum') )
    {
        ...
        if ( ref($peptide)
            && $peptide->isa('InSilicoSpectro::InSilico::Peptide') )
        {
    ...
    @{ $table->{splitPept} } =
      annotatePept( $spectrum->{peptide}, $spectrum->{modif}, $modifLvl );
    $table->{intSel}      = $intSel;
    $table->{modifLvl}    = $modifLvl;
    $table->{numExpPeaks} = defined($expSpectrum) ? scalar(@$expSpectrum) : 0;
    my $len     = length( $table->{peptide} );
    my $normInt = {};
    normalizeIntensities( $intSel, $expSpectrum, $normInt, $massIndex,
        $intensityIndex )
    ...
    foreach my $frag ( sort $cmp keys( %{ $spectrum->{mass}{term} } ) ) {
        ...
        for ( my $i = 0 ; $i < @{ $spectrum->{ionType}{$frag} } ; $i++ ) {
            ...
            # Fragment type name
            push( @{ $table->{mass}{term}[$fragNum] }, $frag )
              ;    #$spectrum->{ionType}{$frag}[$i]);
            if ( defined($expSpectrum) ) {
                push( @{ $table->{match}{term}[$fragNum] }, $frag )
                  ;    #$spectrum->{ionType}{$frag}[$i]);
                push( @{ $table->{intens}{term}[$fragNum] }, $frag )
                  ;    #$spectrum->{ionType}{$frag}[$i]);
            }
                ...
                for ( my $j = $i * $len ; $j < ( $i + 1 ) * $len ; $j++ ) {
                    if (
                        defined(
                            (
                                my $theoMass =
                                  $spectrum->{mass}{term}{$frag}[$j]
                            )
                        )
                      )
                    {
                        push(
                            @{ $table->{mass}{term}[$fragNum] },
                            sprintf( $format, $theoMass )
                        );
                        if ( defined($expSpectrum) ) {
                            my $noMatch = 1;
                            if (
                                defined( $spectrum->{match}{term}{$frag}[$j] ) )
                            {

                                # Found a match, checks precision
                                my $expMass =
                                  $spectrum->{match}{term}{$frag}[$j][0];
                                if (
                                    !defined($tol)
                                    || (
                                        abs( $theoMass - $expMass ) /
                                        ( $theoMass + $expMass ) *
                                        2e+6 <= $tol )
                                    || (
                                        abs( $theoMass - $expMass ) <= $minTol )
                                  )
                                {

                                    # Precision ok or no precision required
                                    push(
                                        @{ $table->{match}{term}[$fragNum] },
                                        sprintf( $format, $expMass )
                                    );
                                    push(
                                        @{ $table->{intens}{term}[$fragNum] },
                                        sprintf(
                                            "%.2f",
                                            $normInt->{
                                                $spectrum->{match}{term}{$frag}
                                                  [$j][0]
                                              }
                                        )
                                    );
                        ...
                        if ( defined($expSpectrum) ) {
                            ...
                            push( @{ $table->{intens}{term}[$fragNum] },
                                undef );
                        }
                    }
                }
    ...
    $fragNum = 0;
    foreach
      my $frag ( sort cmpFragTypes keys( %{ $spectrum->{mass}{intern} } ) )
    {
        push(
            @{ $table->{mass}{intern}[$fragNum] },
            $spectrum->{ionType}{$frag}[0]
        );
        if ( defined($expSpectrum) ) {

            # Name of the fragment
            push(
                @{ $table->{match}{intern}[$fragNum] },
                $spectrum->{ionType}{$frag}[0]
            );
            push(
                @{ $table->{intens}{intern}[$fragNum] },
                $spectrum->{ionType}{$frag}[0]
            );
        }
        foreach my $aa ( sort keys( %{ $spectrum->{mass}{intern}{$frag} } ) ) {
            my $theoMass = $spectrum->{mass}{intern}{$frag}{$aa};
            push( @{ $table->{mass}{intern}[$fragNum] },
                $aa, sprintf( $format, $theoMass ) );
            if ( defined($expSpectrum) ) {
                my $noMatch = 1;
                if ( defined( $spectrum->{match}{intern}{$frag}{$aa} ) ) {
                      ...
                      )
                    {
                        ...
                        # Precision ok or no precision required
                        push( @{ $table->{match}{intern}[$fragNum] },
                            $aa, sprintf( $format, $expMass ) );
                        push(
                            @{ $table->{intens}{intern}[$fragNum] },
                            $aa,
                            sprintf( "%.2f",
                                $normInt->{ $spectrum->{match}{intern}{$frag}
                                      {$aa}[0] } )
                        );
                        $noMatch = 0;
                    }
                ...
                if ($noMatch) {
                    ...
                    # Maintain identical indices by adding undefs
                    push( @{ $table->{match}{intern}[$fragNum] }, undef,
                        undef );
                    push( @{ $table->{intens}{intern}[$fragNum] },
                        undef, undef );
                }
            }
        }
...
sub tabSepSpectrum {
    ...
    # Title
    my $string =
      join( '', annotatePept( $table->{peptide}, $table->{modif}, 2 ) )
      . "\n$table->{peptideMass}\n";
    ...
    for ( my $i = 0 ; $i < @{ $table->{mass}{term} } ; $i++ ) {
        ...
        for ( my $j = 1 ; $j < @{ $table->{mass}{term}[$i] } ; $j++ ) {
            ...
            if ( defined( $table->{match}{term}[$i][$j] ) ) {
                $string .=
                  "($table->{match}{term}[$i][$j],$table->{intens}{term}[$i][$j])";
            }
        }
        $string .= "\n";
    }
    ...
    for ( my $i = 0 ; $i < @{ $table->{mass}{intern} } ; $i++ ) {
        for ( my $j = 1 ; $j < @{ $table->{mass}{intern}[$i] } ; $j += 2 ) {
            push(
                @tmp,
                "$table->{mass}{intern}[$i][0]\t$table->{mass}{intern}[$i][$j]\t$table->{mass}{intern}[$i][$j+1]"
                  . (
                    defined( $table->{match}{intern}[$i][ $j + 1 ] )
                    ? "($table->{match}{intern}[$i][$j+1],$table->{intens}{intern}[$i][$j+1])"
                    : ''
                  )
            );
        }
...
sub latexSpectrum {
    ...
    $string .=
        'Ion types & Residues & Mass'
      . ( ' & Ion types & Residues & Mass' x ( $nColIntern - 1 ) )
      . " \\\\ \\hline\n";
    my @tmp;
    for ( my $i = 0 ; $i < @{ $table->{mass}{intern} } ; $i++ ) {
        for ( my $j = 1 ; $j < @{ $table->{mass}{intern}[$i] } ; $j += 2 ) {
            my ( $tmp1, $tmp2 ) =
              ( $table->{mass}{intern}[$i][0], $table->{mass}{intern}[$i][$j] );
            $tmp1 =~ s/([\$&%#_\{\}])/\\$1/g;
            $tmp2 =~ s/([\$&%#_\{\}])/\\$1/g;
...
sub htmlTerm {
    ...
    my (%h) = @_;
    my (
        $colLineFunc, $bgTitle,  $boldTitle, $bgFrag,
        $boldFrag,    $lineCol1, $lineCol2,  $css
      )
      = (
        $h{colLineFunc}, $h{bgTitle},  $h{boldTitle}, $h{bgFrag},
        $h{boldFrag},    $h{lineCol1}, $h{lineCol2},  $h{css}
      );
    ...
    for ( my $i = 0 ; $i < @{ $table->{splitPept} } ; $i++ ) {
        ...
        if ( defined($css) ) {
            ...
            $string .=
              "<td align=\"center\" bgcolor=\"$bgTitle\">$bBoldTitle$tmp$eBoldTitle</td>";
        }
    }
    $string .= "\n</tr>\n";
    my $prevFrag;
    ...
    for ( my $i = 0 ; $i < @{ $table->{mass}{term} } ; $i++ ) {
        ...
        if ( defined($css) ) {
            ...
            $string .=
              "  <td align=\"left\" bgcolor=\"$bgFrag\">$bBoldFrag$tmp$eBoldFrag</td>";
        }
        for ( my $j = 1 ; $j < @{ $table->{mass}{term}[$i] } ; $j++ ) {
            &$colLineFunc( $table->{mass}{term}[$i][0],
                $prevFrag, \$currentColor, \$oldColor );
            $prevFrag = $table->{mass}{term}[$i][0];
            if ( defined($css) ) {
                $string .= "<td $currentColor>"
                  . (
                    defined( $table->{mass}{term}[$i][$j] )
                    ? $table->{mass}{term}[$i][$j]
                    : '&nbsp;'
                  ) . "</td>";
            }
            else {
                $string .= "<td align=\"right\" bgcolor=\"$currentColor\">"
                  . (
                    defined( $table->{mass}{term}[$i][$j] )
                    ? $table->{mass}{term}[$i][$j]
                    : '&nbsp;'
...
sub htmlIntern {
    my $table = shift;
    my (%h) = @_;
    my ( $bgTitle, $boldTitle, $bgFrag, $boldFrag, $bgIntern, $nColIntern,
        $css ) = (
        $h{bgTitle},  $h{boldTitle},  $h{bgFrag}, $h{boldFrag},
        $h{bgIntern}, $h{nColIntern}, $h{css}
        );
    ...
    if ( defined($css) ) {
        $string .=
          "<tr>\n  "
          . (
            "<td class=\"massOutputColTitleFrag\">Ion types</td><td class=\"massOutputColTitle\">Residues</td><td class=\"massOutputColTitle\">Mass</td>"
              x $nColIntern )
          . "\n</tr>\n";
    }
    else {
        $string .=
          "<tr>\n  "
          . (
            "<td align=\"left\" bgcolor=\"$bgFrag\">$bBoldTitle Ion types$eBoldTitle</td><td align=\"center\" bgcolor=\"$bgTitle\">$bBoldTitle Residues$eBoldTitle</td><td align=\"center\" bgcolor=\"$bgTitle\">$bBoldTitle Mass$eBoldTitle</td>"
              x $nColIntern )
          . "\n</tr>\n";
    }
    ...
    for ( my $i = 0 ; $i < @{ $table->{mass}{intern} } ; $i++ ) {
        for ( my $j = 1 ; $j < @{ $table->{mass}{intern}[$i] } ; $j += 2 ) {
            ...
            if ( defined($css) ) {
                push( @tmp,
                    "<td class=\"massOutputFragName\">$tmp1</td><td class=\"massOutputInternCenter\">$tmp2</td><td class=\"massOutputInternRight\">$table->{mass}{intern}[$i][$j+1]</td>"
                );
            }
            else {
                push( @tmp,
                    "<td align=\"left\" bgcolor=\"$bgFrag\">$bBoldFrag$tmp1$eBoldFrag</td><td align=\"center\" bgcolor=\"$bgIntern\">$tmp2</td><td align=\"right\" bgcolor=\"$bgIntern\">$table->{mass}{intern}[$i][$j+1]</td>"
                );
            }
        }
    }
    ...
    if ( $n % $nColIntern != 0 ) {
        ...
        for ( my $i = 0 ; $i < $remain ; $i++ ) {
            if ( defined($css) ) {
                $string .=
                  "<td class=\"massOutputFragName\">&nbsp;</td><td class=\"massOutputInternCenter\">&nbsp;</td><td class=\"massOutputInternRight\">&nbsp;</td>";
            }
            else {
                $string .=
                  "<td align=\"left\" bgcolor=\"$bgFrag\">&nbsp;</td><td align=\"center\" bgcolor=\"$bgIntern\">&nbsp;</td><td align=\"right\" bgcolor=\"$bgIntern\">&nbsp;</td>";
            }
        }
        $string .= "\n</tr>\n";
    }
...
sub plotSpectrumMatch {
    croak "cannot call graphic method when GD module coul not be loaded"
      unless $okGD;
    ...
    my (%h) = @_;
    my (
        $fname,     $fhandle,      $fontChoice,       $colorScale,
        $format,    $inCellBorder, $bgColor,          $textColor,
        $lineColor, $modifAAColor, $changeColModifAA, $style,
        $legend,    $nColIntern,   $plotIntern
      )
      = (
        $h{fname},        $h{fhandle},          $h{fontChoice},
        $h{colorScale},   $h{format},           $h{inCellBorder},
        $h{bgColor},      $h{textColor},        $h{lineColor},
        $h{modifAAColor}, $h{changeColModifAA}, $h{style},
        $h{legend},       $h{nColIntern},       $h{plotIntern}
      );
    ...
    my (
        $fontWidth, $fontHeight, $font, $fontName,
        $fontPoint, $ttHShift,   $ttVShift
    );
    ...
    if ( $class eq 'default' ) {
        ...
        if ( -e $class ) {
            ...
            foreach ( split( //, 'ACDEFGHIKLMNPQRSTVWYabcbxyz*#°' ) ) {
                @coord =
                  $tmpim->stringFT( $tmpWhite, $fontName, $fontPoint, 0, 10,
                    180, $_ );
            ...
            @coord =
              $tmpim->stringFT( $tmpWhite, $fontName, $fontPoint, 0, 10, 180,
                'AFGHKMPQRSWYbyz*#°' );
    ...
    # Legend box
    my (
        $legendHeight, $legendWidth, @legend,
        @intThres,     @countPeaks,  @countMatched
    );
    ...
    if ( defined($legend) ) {
        ...
        for ( my $i = 0 ; $i < @{ $table->{mass}{term} } ; $i++ ) {
            for ( my $j = 1 ; $j < @{ $table->{mass}{term}[$i] } ; $j++ ) {
                if (
                    defined( ( my $expMass = $table->{match}{term}[$i][$j] ) ) )
                {
                    if ( !$already{$expMass} ) {
                        $countMatched[
                          selectColor( \@intThres,
                              $table->{intens}{term}[$i][$j] )
                        ]++;
                    }
                    $already{$expMass} = 1;
                }
        ...
        for ( my $i = 0 ; $i < @intThres ; $i++ ) {
            push( @countLegend,
                ( $countMatched[$i] + 0 ) . '/' . ( $countPeaks[$i] + 0 ) );
            $maxCountLen = length( $countLegend[-1] )
              if ( length( $countLegend[-1] ) > $maxCountLen );
        }
        ...
        if ( defined($font) ) {
            ...
            $legendWidth =
              ( $maxLegLen + $maxCountLen ) * $nativeFontWidth +
              4 * $inCellBorder + 3;
            $countStartPos =
              $maxLegLen * $nativeFontWidth + 2 * $inCellBorder + 1;
        }
        else {
    ...
    if ( defined($plotIntern) ) {
        ...
        for ( my $i = 0 ; $i < @{ $table->{mass}{intern} } ; $i++ ) {
            for ( my $j = 1 ; $j < @{ $table->{mass}{intern}[$i] } ; $j += 2 ) {
                push(
                    @internList,
                    [
                        $table->{mass}{intern}[$i][0],
                        $table->{mass}{intern}[$i][$j],
                        defined( $table->{match}{intern}[$i][ $j + 1 ] )
                        ? $table->{intens}{intern}[$i][ $j + 1 ]
                        : undef
                    ]
        ...
        for ( my $i = 0 ; $i < @internList ; $i++ ) {
            $maxFrag = length( $internList[$i][0] )
              if ( length( $internList[$i][0] ) > $maxFrag );
            $maxAA = length( $internList[$i][1] )
              if ( length( $internList[$i][1] ) > $maxAA );
        }
        @internHPos = (0);
        for ( my $i = 0 ; $i < $nColIntern ; $i++ ) {
            push( @internHPos,
                $internHPos[-1] +
                  ...
                  1 );
            push( @internHPos,
                $internHPos[-1] + $maxAA * $fontWidth + 2 * $inCellBorder + 1 );
        }
        ...
        push( @internVPos, $internVPos[-1] + $cellHeight )
          unless ( scalar(@internList) % $nColIntern == 0 );
    }
    ...
    if ( $legend eq 'right' ) {
        ...
        if ( defined($plotIntern) ) {
            ...
            $imageWidth  = $internHPos[-1] + 1
              if ( $internHPos[-1] + 1 > $imageWidth );
        }
    }
    elsif ( $legend eq 'bottom' ) {
        if ( defined($plotIntern) ) {
            if ( $internHPos[-1] + 2 * $inCellBorder + $legendWidth + 1 >
                $imageWidth )
            {

                # Enlarge image because of overlap
                $imageWidth =
                  $internHPos[-1] + 2 * $inCellBorder + $legendWidth + 1;
            }
            $imageHeight =
              max( $imageHeight + $legendHeight + 2 * $inCellBorder - 1,
                $internVPos[-1] + 1 );
        }
        else {
    ...
    $modifAAColor =
      defined($modifAAColor) ? $im->colorAllocate(@$modifAAColor) : $blue;
    ...
    for ( my $i = 0 ; $i < @fragNames ; $i++ ) {
        if ( defined($font) ) {
            $im->string(
                $font,
                $inCellBorder + 1,
                $hLines[ $i + 1 ] + $inCellBorder + 1,
                $fragNames[$i], $textColor
            );
        }
        else {
            $im->stringFT(
                ...
                $fontPoint,
                0,
                $inCellBorder + 1 + $ttHShift,
                $hLines[ $i + 1 ] + $inCellBorder + 1 + $ttVShift,
                $fragNames[$i]
            );
        }
    }
        ...
        if ( $modified[$i] == 0 ) {
            ...
            if ( defined($font) ) {
                ...
                $im->stringFT(
                    ...
                    $fontPoint,
                    0,
                    $vLines[ $i + 1 ] + $inCellBorder + 1 + $ttHShift,
                    $inCellBorder + 1 + $ttVShift,
                    $aa[$i]
                );
            }
        }
            ...
            $im->line(
                $vLines[ $i + 1 ] + $inCellBorder + 1,
                2, $vLines[ $i + 2 ] - $inCellBorder - 1,
                2, $textColor
            );
            $im->line(
                $vLines[ $i + 1 ] + $inCellBorder + 1,
                3, $vLines[ $i + 2 ] - $inCellBorder - 1,
                3, $textColor
            );
        }
    }
    ...
    if ( $style eq 'circle' ) {
        $im->line( $vLines[1], $hLines[1], $vLines[1], $hLines[-1],
            $lineColor );
        ...
        for ( my $i = 0 ; $i < @{ $table->{mass}{term} } ; $i++ ) {
            for ( my $j = 1 ; $j < @{ $table->{mass}{term}[$i] } ; $j++ ) {
                if ( defined( $table->{mass}{term}[$i][$j] ) ) {
                    ...
                    $im->line(
                        $vLines[$j] + $hRadius - 2,
                        $hLines[ $i + 1 ] + $vRadius - 2,
                        $vLines[$j] + $hRadius + 3,
                        $hLines[ $i + 1 ] + $vRadius + 3,
                        $lineColor
                    );
                }
            }
    ...
    if ( defined($legend) ) {
        my ( $legLeft, $legTop ) =
            ( $legend eq 'right' )
          ? ( $vLines[-1] + 2 * $inCellBorder, $hLines[-1] - $legendHeight + 1 )
          : ( $imageWidth - 1 - $legendWidth, $hLines[-1] + 2 * $inCellBorder );
        my $countRight = $legLeft + $legendWidth - $inCellBorder;
        my $legRight   = $legLeft + $countStartPos - $inCellBorder;
        $im->rectangle(
            $legLeft, $legTop,
            $legLeft + $legendWidth,
            $legTop + $legendHeight - 1, $lineColor
        );
        ...
        for ( my $i = 0 ; $i < @legend ; $i++ ) {
            $im->filledRectangle(
                $legLeft + 1,
                $legTop + $i * $cellHeight + 1,
                $legLeft + $countStartPos,
                $legTop + ( $i + 1 ) * $cellHeight - 1,
                $color[ $n - $i ]
            );
            if ( defined($font) ) {
                $im->string(
                    $font,
                    $legRight - length( $legend[ $n - $i ] ) * $nativeFontWidth,
                    $legTop + $i * $cellHeight + $inCellBorder + 1,
                    $legend[ $n - $i ],
                    $legendColor[ $n - $i ]
                );
                $im->string(
                    $font,
                    $countRight -
                      length( $countLegend[ $n - $i ] ) * $nativeFontWidth,
                    $legTop + $i * $cellHeight + $inCellBorder + 1,
                    $countLegend[ $n - $i ],
                    $textColor
                );
                ...
                $im->stringFT(
                    ...
                    0,
                    $legRight - $length,
                    $legTop + $i * $cellHeight + $inCellBorder + 1 + $ttVShift,
                    $legend[ $n - $i ]
                );
                @coord = $tmpim->stringFT(
                    $tmpWhite,
    ...
    if ( defined($plotIntern) ) {
        ...
        for ( my $i = 0 ; $i < @internVPos ; $i++ ) {
            $im->line( 0, $internVPos[$i], $internHPos[-1], $internVPos[$i],
                $lineColor );
        }
        for ( my $i = 0 ; $i < @internHPos ; $i++ ) {
            $im->line(
                $internHPos[$i], $internVPos[0], $internHPos[$i],
                $internVPos[-1], $lineColor
            );
        }
        ...
        for ( my $i = 0 ; $i < @internList ; $i += $nColIntern, $line++ ) {
              ...
              )
            {
                if ( defined($font) ) {
                    $im->string(
                        $font,
                        $internHPos[$col] + $inCellBorder + 1,
                        $internVPos[$line] + $inCellBorder + 1,
                        $internList[ $i + $j ][0],
                        $textColor
                    );
                }
                    ...
                    $im->stringFT(
                        ...
                        $fontPoint,
                        0,
                        $internHPos[$col] + $inCellBorder + 1 + $ttHShift,
                        $internVPos[$line] + $inCellBorder + 1 + $ttVShift,
                        $internList[ $i + $j ][0]
                    );
                }
                if ( defined( $internList[ $i + $j ][2] ) ) {
                    my $index =
                      selectColor( \@intThres, $internList[ $i + $j ][2] );
                    $im->filledRectangle(
                        $internHPos[ $col + 1 ] + 1,
                        $internVPos[$line] + 1,
                        $internHPos[ $col + 2 ] - 1,
                    ...
                    if ( defined($font) ) {
                        $im->string(
                            $font,
                            $internHPos[ $col + 1 ] + $inCellBorder + 1,
                            $internVPos[$line] + $inCellBorder + 1,
                            $internList[ $i + $j ][1],
                            $legendColor[$index]
                        );
    ...
    if ( defined($format) ) {
        ...
        if ($fhandle) {
            ...
            $fname =~ s/\.$format$//;
            open( FGD, ">$fname.$format" )
              || croak("Cannot open file [$fname.$format]: $!");
            binmode FGD;
            print FGD $im->$format;
            close(FGD);
        }
...
sub normalizeIntensities {
    ...
    if ( $intSel eq 'original' ) {
        for ( my $i = 0 ; $i < @$expSpectrum ; $i++ ) {
            $normInt->{ $expSpectrum->[$i][$massIndex] } =
              $expSpectrum->[$i][$intensityIndex];
        }
    }
    elsif ( $intSel eq 'log' ) {
        for ( my $i = 0 ; $i < @$expSpectrum ; $i++ ) {
            $normInt->{ $expSpectrum->[$i][$massIndex] } =
              ( $expSpectrum->[$i][$intensityIndex] > 1 )
              ? log( $expSpectrum->[$i][$intensityIndex] )
              : 0;
        }
    }
        ...
        for ( my $i = 0 ; $i < @$expSpectrum ; $i++ ) {
            $normInt->{ $expSpectrum->[$i][$massIndex] } =
              $expSpectrum->[$i][$intensityIndex] / $maxIntensity;
        }
    }
        ...
        if ( @$expSpectrum > 1 ) {
            ...
            for ( my $i = 0 ; $i < @$expSpectrum ; $i++ ) {
                push( @sortedInt, $expSpectrum->[$i][$intensityIndex] )
                  unless ( $already{ $expSpectrum->[$i][$intensityIndex] } )
                  ;    # Each intensity is added once
                push(
                    @{ $backHash{ $expSpectrum->[$i][$intensityIndex] } },
                    $expSpectrum->[$i][$massIndex]
                );     # List of the masses sharing the same intensity
...
sub htmlCSS {
    my (%h) = @_;
    my (
        $bgTitle,  $boldTitle, $bgFrag, $boldFrag,
        $lineCol1, $lineCol2,  $bgIntern
      )
      = (
        $h{bgTitle},  $h{boldTitle}, $h{bgFrag}, $h{boldFrag},
        $h{lineCol1}, $h{lineCol2},  $h{bgIntern}
      );
    ...
    $string .=
      "td.massOutputColTitleFrag {text-align: left; background-color: $bgFrag";
    $string .= '; font-weight: bold' if ( defined($boldTitle) );
    $string .= "}\n";

    $string .=
      "td.massOutputColTitle {text-align: center; background-color: $bgTitle";
    $string .= '; font-weight: bold' if ( defined($boldTitle) );
    $string .= "}\n";

    $string .=
      "td.massOutputFragName {text-align: left; background-color: $bgFrag";
    $string .= '; font-weight: bold' if ( defined($boldFrag) );
    $string .= "}\n";

    $string .=
      "td.massOutputLineCol1 {text-align: right; background-color: $lineCol1}\n";
    $string .=
      "td.massOutputLineCol2 {text-align: right; background-color: $lineCol2}\n";
    $string .=
      "td.massOutputInternCenter {text-align: center; background-color: $bgIntern}\n";
    $string .=
      "td.massOutputInternRight {text-align: right; background-color: $bgIntern}\n";
...
sub plotLegendOnly {
    my (%h) = @_;
    my ( $fname, $fhandle, $fontChoice, $intSel, $colorScale, $format,
        $inCellBorder, $lineColor )
      = (
        $h{fname},      $h{fhandle}, $h{fontChoice},   $h{intSel},
        $h{colorScale}, $h{format},  $h{inCellBorder}, $h{lineColor}
      );
    ...
    for ( my $i = 0 ; $i < @legend ; $i++ ) {
        ...
        if ( defined($font) ) {
            $im->string(
                ...
                $legendWidth -
                  $inCellBorder - 1 -
                  length( $legend[ $n - $i ] ) * $fontWidth,
                $i * $cellHeight + $inCellBorder + 1,
                $legend[ $n - $i ],
                $legendColor[ $n - $i ]
            );
        }
            ...
            $im->stringFT(
                ...
                $fontPoint,
                0,
                $legendWidth - $inCellBorder - 2 - $length + $ttHShift,
                $i * $cellHeight + $inCellBorder + 1 + $ttVShift,
                $legend[ $n - $i ]
            );
        }
    }

...
sub new {
    ...
    $ec->set( 'gamma', 1e-6 )
      ;                 # Filter predicted values which are closer than gamma
    $ec->set( 'expdata', [] );
    $ec->set( 'prdata',  [] );
...
sub train {
    ...
    for ( $this->{fitting} ) {
        ...
        /linear/ && do {
            my $reg = Statistics::Regression->new( "linear regression",
                [ "const", "prdata" ] );
            foreach (@expdata) {
        ...
        /spline/ && do {
            $this->{fitmodel} = Math::Spline->new( \@prdata, \@expdata );
            last;
        };
...
sub fit {
    ...
    for ( $this->{fitting} ) {
        /bypass/
          && do { $this->set( 'fitmodel', [ $a, $b ] ); return ( $a + $b * $x ); };
        /linear/
          && do { ( $a, $b ) = $this->{fitmodel}->theta; return ( $a + $b * $x ); };
...
sub preprocess
{    # clean outliers and inconsistencies bw predicted and experimental data

    my ( $this, $x, $y ) = @_;

    for ( my ( $k, $no ) = ( 0, 1 ) ; $k < ( scalar @{$x} - 1 ) ; $k++ )
    {    # Filter of repeated predicted values
        if ( abs( ${$x}[$k] - ${$x}[ $k + 1 ] ) < $this->{gamma} ) {
            splice( @{$x}, $k, 1 );    # Repeated predicted value: leave it out
            ${$y}[$k] *= $no++;
            ${$y}[$k] += splice( @{$y}, $k + 1, 1 );
            ;                          # Take the mean of experimental values
            ${$y}[ $k-- ] /= $no;
        }
        else {
            $no = 1;

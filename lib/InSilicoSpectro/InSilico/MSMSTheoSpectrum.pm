...
sub new {
    ...
    if ( ref( $_[0] )
        && $_[0]->isa('InSilicoSpectro::InSilico::MSMSTheoSpectrum') )
    {
        %$tsp = %{ $_[0] };
        bless( $tsp, $class );
    }
...
sub getTermIons {
    ...
    foreach my $frag ( keys( %{ $spectrum{mass}{term} } ) ) {
        for ( my $i = 0 ; $i < @{ $spectrum{ionType}{$frag} } ; $i++ ) {
            my ( $series, $charge ) =
              ( InSilicoSpectro::InSilico::MassCalculator::getFragType($frag) )
              [ 0, 1 ];
            my $terminus =
              ( InSilicoSpectro::InSilico::MassCalculator::getSeries($series) )
              [0];
            ...
            push(
                @termIons,
                new InSilicoSpectro::InSilico::TermIonSeries(
                    ionType        => $ionType,
                    charge         => $charge,
                    series         => $series,
                    terminus       => $terminus,
    ...
    @termIons = sort {
        InSilicoSpectro::InSilico::MSMSOutput::cmpFragTypes( $a->ionType(),
            $b->ionType() )
    } @termIons;
    return @termIons;
...
sub getInternIons {
    ...
    foreach my $frag ( keys( %{ $spectrum{mass}{intern} } ) ) {
        ...
        push(
            @internIons,
            new InSilicoSpectro::InSilico::InternIonSeries(
                ionType        => $ionType,
                charge         => 1,
                names          => [@names],
                masses         => [@masses],

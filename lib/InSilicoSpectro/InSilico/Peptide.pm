...
sub parentProtein {
    ...
    if ( defined($val) ) {
        if ( $val->isa('InSilicoSpectro::InSilico::AASequence') ) {
            ...
            croak(
                "The object must be of class InSilicoSpectro::InSilico::AASequence ["
                  . ref($val)
                  . "]" );
        }
    }
...
sub sequence {
    ...
    if ( defined( $this->{sequence} ) ) {
        ...
        if (   !defined( $this->{start} )
            || !defined( $this->{end} )
            || !defined( $this->{parentProtein} ) )
        {
            croak(
                "All of parentProtein, start, end must be defined if no sequence is set"
            );
        }
        if ( $this->{start} < $this->{end} ) {

            # Direct orientation
            return substr( $this->parentProtein->sequence,
...

{
    ...
    if ( defined($modif) ) {
        if ( ref($modif) eq 'ARRAY' ) {
            ...
            $this->{modifType} =
              (      ( length( $modif->[0] ) > 0 )
                  && ( $modif->[0] eq int( $modif->[0] ) ) ) ? 'PMF' : 'MS/MS';
        }
        elsif ( !ref($modif) ) {
...
sub getMass {
    ...
    if ( my $enz = $this->enzyme() ) {
        if (
               $enz->NTermGain()
            && !$this->nTerm()
            && (   ( $this->enzymatic() eq 'full' )
                || ( $this->enzymatic() eq 'half-N' ) )
          )
        {
            ...
            # Standard +H rule
            $termGainMass +=
              InSilicoSpectro::InSilico::MassCalculator::getMass('el_H');
        }
        if (
               $enz->CTermGain()
            && !$this->cTerm()
            && (   ( $this->enzymatic() eq 'full' )
                || ( $this->enzymatic() eq 'half-C' ) )
          )
        {
            ...
            $termGainMass +=
              InSilicoSpectro::InSilico::MassCalculator::getMass('el_H') +
              InSilicoSpectro::InSilico::MassCalculator::getMass('el_O');
        }
    }
    ...
    $mass += InSilicoSpectro::InSilico::MassCalculator::getMass('el_H+')
      if ( $this->addProton() );
    $this->{mass} = $mass;
    $this->{massType} =
      InSilicoSpectro::InSilico::MassCalculator::getMassType();
...
sub getMoZ {
    ...
    my $m      = $this->getMass();
    $m -= InSilicoSpectro::InSilico::MassCalculator::getMass('el_H+')
      if ( $this->addProton() );
    return $m / $charge +
      InSilicoSpectro::InSilico::MassCalculator::getMass('el_H+');
}
...
sub getLength {
    ...
    return defined( $this->{sequence} )
      ? length( $this->{sequence} )
      : abs( $this->{posEnd} - $this->{posStart} ) + 1;
...
sub toString {
    my $this = shift;
    return
        ( length( $this->aaBefore() ) == 1 ? $this->aaBefore() . '.' : '' )
      . $this->sequence()
      . ( length( $this->aaAfter() ) == 1 ? '.' . $this->aaAfter() : '' );
}
...
sub print {
    ...
    print $fdOut ""
      . (
        InSilicoSpectro::InSilico::MassCalculator::modifToString(
            $this->modif()
        )
      )
      . "\n"

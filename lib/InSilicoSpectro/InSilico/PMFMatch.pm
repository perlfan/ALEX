...
@ISA = qw(Exporter);
@EXPORT =
  qw($pmfMatchPeptideIndex $pmfMatchPeakIndex $pmfMatchErrorIndex $pmfMatchRelErrorIndex);
@EXPORT_OK = ();
...
sub getMatchedPeaks {
    ...
    return undef
      if ( !defined( $this->match() )
        || !defined( $this->digestResult() )
        || !defined( $this->expSpectrum() ) );

    $minTol = $minTol || 0.1;
    my $massIndex =
      $this->expSpectrum()->peakDescriptor()->getFieldIndex('mass');
        ...
        if ( $match[$i] ) {
            ...
            if ( defined($tol) ) {
                ...
                push(
                    @matched,
                    [
                        $this->{digestResult}[$i], $match[$i], $error, $relError
                    ]
                );
            }
        }

...
@EXPORT =
  qw($REMOVE_TEMP_FILES $QUEUE_MAX_LEN &verbose &queueMaxSize &dump_all $keyCpt);
@EXPORT_OK = ();
...
sub FC_tempdir {
    unless ($tempDir) {
        $tempDir = tempdir(
            File::Spec->tmpdir() . "/File-Cached-XXXXXX",
            CLEANUP => $REMOVE_TEMP_FILES,
            UNLINK  => $REMOVE_TEMP_FILES
        );
...
sub FC_load {
    my $self  = shift;
    my $fname = $self->FC_file
      or croak "cannot FC_load an object when file does not exits";
    my $serializer = Data::Serializer->new(%serializeParams);
    my $h          = $serializer->retrieve($fname)
      or croak "could not retrieved from serialized file [$fname]: $!";
    foreach ( keys %$h ) {
        $self->{$_} = $h->{$_};
    }

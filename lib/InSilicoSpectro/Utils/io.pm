...
@EXPORT =
  qw(&getFD $VERBOSE &compressFile &uncompressFile &zipFiles &getMD5 &croakIt &isInteractive);
@EXPORT_OK = ();
...
sub uncompressFile {
    ...
    croak "try to gunzip a file without a .gz suffix [$fname]"
      unless $fname =~ /^(.*)\.gz$/i;
...
sub headGz {
    ...
    my @res;
    croakIt("try to gunzip a file without a .gz suffix [$fname]")
      unless $fname =~ /^(.*)\.gz$/i;
...
sub zipFiles {
    ...
    if ( ( !defined $dest ) || ( ( ref $dest ) eq 'GLOB' ) ) {
        new SelectSaver( InSilicoSpectro::Utils::io->getFD($dest) )
          if defined $dest;
        croak "io::zipFiles($dest, $files): write error"

...
if (
    ...
    || $man
    || ( not defined $srcFile )
    || ( not defined $indexMakerFile )
  )
{

    print STDERR "must define --src=xmlfile" unless defined $srcFile;
    print STDERR "must define --indexmaker=xmlfile"
      unless defined $indexMakerFile;
...
eval {
    ...
    if ( $srcFile =~ /\.gz/i ) {
        ...
        InSilicoSpectro::Utils::io::uncompressFile( $srcFile,
            { remove => 0, dest => $realSrc } );
    }

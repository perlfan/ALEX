...
$protSeq =~ s/\s//g;
my ( @modif, $protein );
if (   ( length($protSeq) < 15 )
    && ( ( $protSeq =~ /[A-Z0-9]+_[A-Z0-9]+/ ) || ( $protSeq =~ /\d+/ ) ) )
{

    # Assumes either an ID or an AC
    eval {
        require Bio::Perl;
        my $bpseq = Bio::Perl::get_sequence( 'swiss', $protSeq )
          || CORE::die("cannot access [$protSeq] on remote DB");
    ...
    for ( my $i = 0 ; $i < @seq ; $i++ ) {
        if ( $seq[$i] eq '{' ) {
              ...
              )
            {
                if (
                    !defined(
                        InSilicoSpectro::InSilico::ModRes::getFromDico($_)
                    )
                  )
                {
...
if ( $enzymatic eq 'fully' ) {
    ...
    @result = nonSpecificDigestion(
        protein   => $protein,
        nmc       => $nmc,
        addProton => $addProton,
        enzyme    => ( $enzymatic eq 'half' ? $enzyme : undef ),
        modif => ( @modifVect > 0 ? \@modifVect : undef ),
        pmf   => ( $pmf           ? 1           : undef ),
        minMass => $minMass,
        maxMass => $maxMass,
...
if ( defined(@massList) ) {
    @match = matchPMF(
        ...
        digestResult   => \@result,
        tol            => ( defined($mostIntense) ? $tol : undef ),
        minTol         => $minTol,
        massIndex      => $massIndex,
...
print
  "<tr><td align=left><b>Peptide</b></td><td align=right><b>Start</b></td><td align=right><b>Stop</b></td><td align=right><b>MC</b></td><td align=right><b>Mass</b></td><td align=left><b>Modifications</b></td>";
if ( defined(@massList) ) {
...
if (@result) {
    for ( my $i = 0 ; $i < @{ $result[0] } ; $i++ ) {
        ...
        if ( defined( $match[$i] ) ) {
            ...
            $error =
              ( $expMass - $theoMass ) / ( $expMass + $theoMass ) * 2.0e6;
            if (   ( abs($error) <= $tol )
                || ( abs( $expMass - $theoMass ) <= $minTol ) )
            {
                $matched = 1;
            }
        }
        ...
        if ($matched) {
            print
              "<tr><td align=left bgcolor=$colMatch>$peptide</td><td align=right>$result[$digestIndexStart][$i]</td><td align=right>$result[$digestIndexEnd][$i]</td><td align=right>$result[$digestIndexNmc][$i]</td><td align=right>",
              sprintf( "%.3f", $result[$digestIndexMass][$i] ),
              "</td><td align=left>", ( $modif || '&nbsp;' ),
              "</td><td align=right bgcolor=$colMatch>",
              sprintf( "%.3f", $error ), "</td>";
            $peptideList{$peptide} = 1;
            if ( !$pmf ) {
                print
                  "<td><input type=submit value=Frag onClick=\"document.digestResForm.peptide.value='$peptide'; document.digestResForm.modif.value='$modif'\"></td>";
            }
            print "</tr>\n";
        }
        elsif ( !defined(@massList) || !$matchedOnly ) {
            print
              "<tr><td align=left>$peptide</td><td align=right>$result[$digestIndexStart][$i]</td><td align=right>$result[$digestIndexEnd][$i]</td><td align=right>$result[$digestIndexNmc][$i]</td><td align=right>",
              sprintf( "%.3f", $result[$digestIndexMass][$i] ),
              "</td><td align=left>", ( $modif || '&nbsp;' ), "</td>";
            $peptideList{$peptide} = 1;
            if ( defined(@massList) ) {
    ...
    print
      "<font color='red'>none sequences could be generated (check param)</font>\n";
}
